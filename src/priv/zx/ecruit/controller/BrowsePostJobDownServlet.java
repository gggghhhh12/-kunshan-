...
public class BrowsePostJobDownServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		//从数据库中查找出该公司发布的职位要求
		try {
				eppjs = eppjd.getEPPostJobAlldown(browseUsername);
		} catch (SQLException e) {
			e.printStackTrace();
		}
		//将职位要求对象放入request中
		request.setAttribute("browseEpDataDowns", eppjs);
		//转向EPModifyPostJob.jsp页面
		request.getRequestDispatcher("../BrowsePostJobDown.jsp").forward(request, response);
	}
